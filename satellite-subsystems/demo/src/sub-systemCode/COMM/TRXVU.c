...
void update_FRAM_bitRate()
{
	...
	if (error != 0)
		WriteErrorLog(LOG_ERR_TRXVU_SET_BITRATE, SYSTEM_TRXVU, error);
	vTaskDelay(1000);
...
void toggle_idle_state()
{
	for (int i = 0; i < 2; i++)
	{
		...
		if (retValInt != 0)
			WriteErrorLog(LOG_ERR_TRXVU_IDLE_STATE, SYSTEM_TRXVU, retValInt);
		vTaskDelay(1000);
...
void TRXVU_init_hardWare()
{
	...
	if (retValInt != 0)
		WriteErrorLog(LOG_ERR_TRXVU_INITIALIZE, SYSTEM_TRXVU, retValInt);
...
void Dump_task(void *arg)
{
	...
	if (f_error != 0)
		WriteErrorLog(LOG_ERR_ENTER_FS, SYSTEM_TRXVU, f_error);
...
void Transponder_task(void *arg)
{
	...
	if (get_system_state(transponder_active_param))
	{
		...
		if (i_error != 0)
			WriteErrorLog(LOG_ERR_ENTER_FS, SYSTEM_TRXVU, i_error);
...
void Rx_logic()
{
	...
	if (i_error != 0)
		WriteErrorLog(LOG_ERR_TRXVU_GET_FRAME_COUNT, SYSTEM_TRXVU, i_error);
	if (RxCounter > 0)
	{
...
void reset_FRAM_TRXVU()
{
	...
	if (i_error != 0)
		WriteErrorLog(LOG_ERR_FRAM_WRITE, SYSTEM_TRXVU, i_error);
...
int TRX_sendFrame(byte* data, uint8_t length)
{
	...
	if (xSemaphoreTake_extended(xIsTransmitting, MAX_DELAY))
	{
		...
		do
		{
			...
			if (i_error != 0)
				WriteErrorLog(LOG_ERR_TRXVU_SEND_FRAM, SYSTEM_TRXVU, i_error);
			retVal = 0;
			if (count > 0)
				vTaskDelay(200);
			if (count % 10 == 1)
...
int TRX_getFrameData(unsigned int *length, byte* data_out)
{
	...
	if (i_error != 0)
		WriteErrorLog(LOG_ERR_TRXVU_GET_FRAM, SYSTEM_TRXVU, i_error);
...
void change_TRXVU_state(Boolean state)
{
	...
	if (i_error != 0)
		WriteErrorLog(LOG_ERR_TRXVU_I2C_TRANSPONDER, SYSTEM_TRXVU, i_error);
