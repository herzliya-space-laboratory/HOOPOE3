...
void Auto_Deploy()
{
	...
	if (time_out_before_deploy(DEPLOY_ATTEMPT_NUMBER_2) == 0)
	{
		...
		deploy_status.isAtemptDone = 1;
		deploy_status.minutesToAttempt = START_MUTE_TIME_MIN;
		i_error = FRAM_write((byte*)&deploy_status, DEPLOY_ANTS_ATTEMPTS_ADDR + DEPLOY_ATTEMPT_NUMBER_2, 1);
		check_int("FRAM_write(DEPLOY_ANTS_ATTEMPTS_ADDR), time_out_before_deploy", i_error);
		printf("ended attempt 2 to deploy ants, side B\n");
	}

	if (time_out_before_deploy(DEPLOY_ATTEMPT_NUMBER_3) == 0)
	{
		...
		deploy_status.isAtemptDone = 1;
		deploy_status.minutesToAttempt = START_MUTE_TIME_MIN;
		i_error = FRAM_write((byte*)&deploy_status, DEPLOY_ANTS_ATTEMPTS_ADDR + DEPLOY_ATTEMPT_NUMBER_3, 1);
		check_int("FRAM_write(DEPLOY_ANTS_ATTEMPTS_ADDR), time_out_before_deploy", i_error);
