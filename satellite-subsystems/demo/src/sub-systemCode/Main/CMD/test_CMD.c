...
void cmd_imageDump_test(Ack_type* type, ERR_type* err, TC_spl cmd)
{
	...
	create_task(imageDump_test, (const signed char * const)"Dump_Task", (unsigned short)(STACK_DUMP_SIZE), (void*)raw, (unsigned portBASE_TYPE)(configMAX_PRIORITIES - 2), xDumpHandle);
