...
void SendAdcsTlm(byte *info, unsigned int length, int subType, boolean isOnlineTelem){
	TM_spl tm;
	time_unix time_now;
...
TroubleErrCode AdcsExecuteCommand(TC_spl *cmd)
{
	...
	switch(sub_type)
	{
		...
		case ADCS_GENRIC_ST:
			...
			SendAdcsTlm(&rv,siseof(int), ADCS_GENRIC_ST, FALSE);
			break;
		...
		case ADCS_SET_MTQ_CONFIG_ST:
			...
			//TODO: send ack with 'rv'
			SendAdcsTlm(&rv,siseof(int), ADCS_SET_MTQ_CONFIG_ST, FALSE);
			break;
		...
		case ADCS_SET_DETUMB_CTRL_PARAM_ST:
			...
			//TODO: send ack with 'rv'
			SendAdcsTlm(&rv,siseof(int), ADCS_SET_DETUMB_CTRL_PARAM_ST, FALSE);
			break;
		...
		case ADCS_SET_YWHEEL_CTRL_PARAM_ST:
			...
			//TODO: send ack with 'rv'
			SendAdcsTlm(&rv,siseof(int), ADCS_SET_YWHEEL_CTRL_PARAM_ST, FALSE);
			break;
		...
		case ADCS_SET_RWHEEL_CTRL_PARAM_ST:
			...
			//TODO: send ack with 'rv'
			SendAdcsTlm(&rv,siseof(int), ADCS_SET_RWHEEL_CTRL_PARAM_ST, FALSE);
			break;
		case ADCS_SET_MOMENT_INTERTIA_ST:
			...
			//TODO: send ack with 'rv'
			SendAdcsTlm(&rv,siseof(int), ADCS_SET_MOMENT_INTERTIA_ST, FALSE);
			break;
		case ADCS_PROD_INERTIA_ST:
			err = AdcsI2cCmdWithID(SET_PROD_INERTIA_CMD_ID,cmd->data,12,&rv);
			//TODO: send ack with 'rv'
			SendAdcsTlm(&rv,siseof(int), ADCS_PROD_INERTIA_ST, FALSE);
			break;
		...
		case ADCS_RATE_SENSOR_OFFSET_ST:
			...
			//TODO: send ack with 'rv'
			SendAdcsTlm(&rv,siseof(int), ADCS_RATE_SENSOR_OFFSET_ST, FALSE);
			break;
		...
		case ADCS_GET_FULL_CONFIG_ST:
			...
			//TODO: send ack with 'rv'
			SendAdcsTlm(&rv,siseof(int), ADCS_GET_FULL_CONFIG_ST, FALSE);
			break;
		case ADCS_DUMMY_FUNC:
			rv = cspaceADCS_getGeneralInfo(ADCS_ID,&data);
			SendAdcsTlm(&rv,siseof(int),ADCS_DUMMY_FUNC, FALSE);
		...
		case ADCS_GET_TLM_BY_ID_ST:
			...
			SendAdcsTlm((byte*)buffer,cmd->data[1],cmd->data[0], FALSE);
			break;
		default:
